...
if [[ $(echo "$LATENCY < 50" | bc -l) -eq 1 ]]; then
    ...
    echo "⚠️  WARN: Redis latency ${LATENCY}ms high (may cause timeouts)"
fi
...
if [[ $BATCH_QUEUE_LEN -eq 0 ]] && [[ $VERIFIED_RESULTS_LEN -eq 0 ]]; then
    ...
    echo "⚠️  WARN: Non-empty queues with timeouts suggest processing failure"
...
if [[ $CONNECTED_CLIENTS -lt $(($MAX_CLIENTS / 2)) ]]; then
    ...
    echo "⚠️  WARN: High connection usage ($CONNECTED_CLIENTS/$MAX_CLIENTS)"
fi
...
if [[ $MAX_MEMORY == "0" ]]; then
    ...
    if [[ $(echo "$MEMORY_PCT < 80" | bc -l) -eq 1 ]]; then
        ...
        echo "⚠️  WARN: Memory pressure ${MEMORY_PCT}% may cause slowdowns"
    fi
else
...
if [[ $SLOWLOG_LEN -eq 0 ]]; then
    ...
    echo "⚠️  WARN: $SLOWLOG_LEN slow queries found"
...
if HEALTH_RESP=$(timeout 10 curl -s "$RELAYER_API/health" 2>/dev/null); then
    if echo "$HEALTH_RESP" | grep -q '"status":"healthy"'; then
        ...
        echo "⚠️  WARN: Relayer health check failed: $HEALTH_RESP"
    fi
else
    echo "⚠️  WARN: Relayer health endpoint unreachable"
fi
